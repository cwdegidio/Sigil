import { Token, TokenType, Position, KEYWORDS, STATUS_VALUES, SigilError } from './types.js'

// ─── Lexer ────────────────────────────────────────────────────────────────────
//
// Produces a flat token stream from source text.
//
// Indentation is handled via a stack. When indent level increases, an INDENT
// token is emitted. When it decreases, one or more DEDENT tokens are emitted
// to return to the matching level. Blank lines are skipped — they do not affect
// the indent stack.
//
// Tabs are a hard parse error. Mixed tabs/spaces are a hard parse error.
// The indent unit is inferred from the first indented line and all subsequent
// indented lines must be multiples of it.
//
// Line tokenization follows the grammar's disambiguation rules:
// - A leading `- ` at line start (after indent) is DASH followed by line content
// - After DASH, the rest of the line is FREE_TEXT or a member-ref sequence
// - `@` is emitted as AT; the parser enforces it only appears in member-refs
// - QUOTED_STR captures double-quoted strings including their delimiters
// ─────────────────────────────────────────────────────────────────────────────
...
export function lex(source: string, file: string): LexResult {
  ...
  let indentUnit: number | null = null // inferred on first indented line

  function pos(line: number, col: number): Position {
    return { file, line, col }
  }

  function parseError(message: string, line: number, col: number): void {
    errors.push({ category: 'PARSE', message, pos: pos(line, col) })
  }

  function pushToken(type: TokenType, value: string, line: number, col: number): void {
  ...
  for (let lineIdx = 0; lineIdx < lines.length; lineIdx++) {
    ...
    if (indent > currentIndent) {
      ...
      if (indentUnit === null) {
        ...
        parseError(
          `Inconsistent indentation: expected a multiple of ${indentUnit} spaces, got ${delta} extra`,
          lineNum,
          indent + 1,
        )
        continue
      ...
      if (indentStack[indentStack.length - 1] !== indent) {
        parseError(
          `Indentation level ${indent} does not match any enclosing block`,
          lineNum,
          indent + 1,
        )
        continue
...
function tokenizeLineContent(
  ...
  function pos(col: number): Position {
    return { file, line: lineNum, col }
  }

  function push(type: TokenType, value: string, col: number): void {
