...
export function parse(source, file) {
    ...
    function consume(type, value, errorMsg) {
        if (check(type, value))
            return advance();
        const t = peek();
        errors.push({
    ...
    function parseMemberItems() {
        ...
        while (check('DASH')) {
            ...
            const ref = { pos: refPos, name: nameTok.value };
            if (check('AT')) {
                advance(); // consume @
                if (!check('VERSION')) {
                    errors.push({
    ...
    function parseIdentitySection() {
        const p = currentPos();
        if (!consumeKeyword('identity'))
            return null;
        if (!consume('COLON'))
            return null;
        skipNewline();
    ...
    //     TermName:
    //         definition: "..."
    function parseVocabularySection() {
        const p = currentPos();
        if (!consumeKeyword('vocabulary'))
            return null;
        if (!consume('COLON'))
            return null;
        skipNewline();
    ...
    // invariants:
    //     - ...
    function parseInvariantsSection() {
        const p = currentPos();
        if (!consumeKeyword('invariants'))
            return null;
        if (!consume('COLON'))
            return null;
        skipNewline();
    ...
    function parsePreconditionsField() {
        const p = currentPos();
    ...
    function parsePostconditionsField() {
        const p = currentPos();
    ...
    function parseSigilDeclaration() {
        const p = currentPos();
        if (!consumeKeyword('sigil'))
            return null;
        if (!check('IDENTIFIER')) {
            errors.push({
                category: 'PARSE',
    ...
    function parseCharterDeclaration() {
        const p = currentPos();
        if (!consumeKeyword('charter'))
            return null;
        if (!check('IDENTIFIER')) {
            errors.push({
                category: 'PARSE',
    ...
    function parseDoctrineDeclaration() {
        const p = currentPos();
        if (!consumeKeyword('doctrine'))
            return null;
        if (!check('IDENTIFIER')) {
            errors.push({
                category: 'PARSE',
