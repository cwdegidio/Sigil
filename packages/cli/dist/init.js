...
export function init(projectName, outputDir) {
    const created = [];
    const skipped = [];
...
export function printInitResult(projectName, result) {
...
export function printAgentResult(agentName, result) {
    if (result.created.length > 0) {
...
export async function initInteractive(outputDir) {
    ...
    const agentResult = agentChoice !== 'none' ? addAgent(agentChoice, outputDir) : null;
